...
def extract_information_from_git(base):
    ...
    print(type(revision))
    print(str(revision))
    print(str(revision, 'utf-8'))

    version['revision'] = str(revision[0:12], 'utf-8')
    version['irevision0'] = "0x" + str(revision[0:8], 'utf-8')
    version['irevision1'] = "0x" + str(revision[8:12], 'utf-8')
    version['productionRelease'] = 'false'

    try:
        identify = subprocess.check_output(
            ["git", "-C", base, "describe", "--abbrev=12", "--tags", "HEAD"])
        identify = str(identify, 'utf-8').split('-')
